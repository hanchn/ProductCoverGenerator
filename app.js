class CoverGenerator {
    constructor() {
        ...
        this.selectedElements = new Set(); // 用于存储多个选中的元素
    ...
    startTextEditing(element) {
        ...
        input.style.top = `${element.y - element.fontSize}px`;
        input.style.width = `${element.width}px`;
        input.style.height = `${element.fontSize}px`;
    ...
    showEditPanel(element) {
        ...
        if (this.selectedElements.size > 1) {
            this.selectedElements.forEach(el => {
                if (el.type === 'text') this.textEditSection.style.display = 'block';
                if (el.type === 'image') this.imageEditSection.style.display = 'block';
                if (el.type === 'shape') this.shapeEditSection.style.display = 'block';
            });
        } else if (element) {
            // 单个元素的情况
    ...
    updateImageEditControls(element) {
        ...
        if (element.borderRadius) {
            ...
            document.getElementById('imageBorderRadius').value = radius;
        ...
        if (element.shadow) {
            const shadow = element.shadow.match(/\d+/g);
            document.getElementById('imageShadowBlur').value = shadow[0];
            document.getElementById('imageShadowOffsetX').value = shadow[1];
            document.getElementById('imageShadowOffsetY').value = shadow[2];
            document.getElementById('imageShadowColor').value = element.shadow.match(/#[0-9a-f]{6}/i)[0];
        }
    }
                ...
                this.currentTemplate.elements.forEach(el => {
                    if (el.type === 'background') {
                        ...
                        if (el.color.startsWith('linear-gradient')) {
                            const gradientColors = this.colorPalette.background.match(/#[0-9a-f]{6}/gi);
                            if (gradientColors && gradientColors.length >= 2) {
                                el.color = `linear-gradient(135deg, ${gradientColors[0]} 0%, ${gradientColors[1]} 100%)`;
                            }
                        } else {
                            el.color = this.colorPalette.background;
                        }
                    } else if (el.type === 'text') {
                        if (el.isTitle) {
                            el.color = this.colorPalette.primary;
                        } else if (el.isSubtitle) {
                            el.color = this.colorPalette.secondary;
                        } else {
                            el.color = this.colorPalette.text;
                        }
                    }
                });
